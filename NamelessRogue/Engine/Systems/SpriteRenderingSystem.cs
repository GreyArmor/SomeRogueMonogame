					...
					foreach (var entity in tileToDraw.GetEntities())
					{
						...
						if (furniture != null && drawable != null)
						{
							switch (drawable.Representation)
							{
								case 'T':
									{
										var modelShift = Matrix.CreateTranslation(-0.5f, -0.5f, 0f);
										tilePosition = new Point2(x + postionOffsetX, y + postionOffsetY);
										var matrix = Constants.ScaleDownMatrix * modelShift * Matrix.CreateTranslation(x * Constants.ScaleDownCoeficient, y * Constants.ScaleDownCoeficient, tileToDraw.ElevationVisual * Constants.ScaleDownCoeficient);
